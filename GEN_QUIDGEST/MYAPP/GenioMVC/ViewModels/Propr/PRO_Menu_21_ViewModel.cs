...
{
	public class PRO_Menu_21_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "6a98d3ce-a8e0-4a43-837c-98a413bb8578"; }
...
FieldRef[] fields = new FieldRef[] { CSGenioApropr.FldCodpropr, CSGenioApropr.FldZzstate, CSGenioApropr.FldCodagent, CSGenioAagent.FldCodagent, CSGenioAagent.FldEmail, CSGenioApropr.FldTitulo, CSGenioApropr.FldFoto, CSGenioApropr.FldPreco };
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="PRO_Menu_21_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public PRO_Menu_21_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			if (!tableConfig.StaticFilters.ContainsKey("filter_PRO_Menu_21_TIPOCONST"))
				tableConfig.StaticFilters.Add("filter_PRO_Menu_21_TIPOCONST", null);

			{
				var groupFilters = CriteriaSet.Or();
				bool filter_PRO_Menu_21_TIPOCONST_1 = false;
				if (tableConfig.StaticFilters["filter_PRO_Menu_21_TIPOCONST"] != null)
					filter_PRO_Menu_21_TIPOCONST_1 = tableConfig.StaticFilters["filter_PRO_Menu_21_TIPOCONST"].Contains("1");
				else if (tableConfig.StaticFilters["filter_PRO_Menu_21_TIPOCONST"] == null)
					filter_PRO_Menu_21_TIPOCONST_1 = true;
				if (filter_PRO_Menu_21_TIPOCONST_1)
				{

				}

				bool filter_PRO_Menu_21_TIPOCONST_2 = false;
				if (tableConfig.StaticFilters["filter_PRO_Menu_21_TIPOCONST"] != null)
					filter_PRO_Menu_21_TIPOCONST_2 = tableConfig.StaticFilters["filter_PRO_Menu_21_TIPOCONST"].Contains("2");
				if (filter_PRO_Menu_21_TIPOCONST_2)
				{
					groupFilters.Equal(CSGenioApropr.FldTipoprop, "a");

				}

				bool filter_PRO_Menu_21_TIPOCONST_3 = false;
				if (tableConfig.StaticFilters["filter_PRO_Menu_21_TIPOCONST"] != null)
					filter_PRO_Menu_21_TIPOCONST_3 = tableConfig.StaticFilters["filter_PRO_Menu_21_TIPOCONST"].Contains("3");
				if (filter_PRO_Menu_21_TIPOCONST_3)
				{
					groupFilters.Equal(CSGenioApropr.FldTipoprop, "m");

				}

				bool filter_PRO_Menu_21_TIPOCONST_4 = false;
				if (tableConfig.StaticFilters["filter_PRO_Menu_21_TIPOCONST"] != null)
					filter_PRO_Menu_21_TIPOCONST_4 = tableConfig.StaticFilters["filter_PRO_Menu_21_TIPOCONST"].Contains("4");
				if (filter_PRO_Menu_21_TIPOCONST_4)
				{
			...
			{
				// EPH
				crs = Models.Propr.AddEPH<CSGenioApropr>(ref u, crs, "ML21");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Propr.AddEPH<CSGenioApropr>(ref u, null, "ML21"));
			}
		...
		public void Load(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioApropr> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet pro_menu_21Conds = CriteriaSet.And();
...
FieldRef[] fields = new FieldRef[] { CSGenioApropr.FldCodpropr, CSGenioApropr.FldZzstate, CSGenioApropr.FldCodagent, CSGenioAagent.FldCodagent, CSGenioAagent.FldEmail, CSGenioApropr.FldTitulo, CSGenioApropr.FldFoto, CSGenioApropr.FldPreco };
			...
			conditions.SubSets.Add(pro_menu_21Conds);
			pro_menu_21Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PRO OVERRQ 21]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioApropr>(m_userContext, false, pro_menu_21Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML21", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PRO OVERRQLSTEXP 21]/
...
// USE /[MANUAL PRO OVERRQLIST 21]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioApropr.GetInformation(), QMVC_POS_RECORD, sorts, pro_menu_21Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioApropr> listing = Models.ModelBase.Where<CSGenioApropr>(m_userContext, false, pro_menu_21Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML21", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapPRO_Menu_21(listing);
		...
		private List<Models.Propr> MapPRO_Menu_21(ListingMVC<CSGenioApropr> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapPRO_Menu_21(row));
					i++;
				}
			}
		...
		private Models.Propr MapPRO_Menu_21(CSGenioApropr row)
		{
		...
		#region Custom code
// USE /[MANUAL PRO VIEWMODEL_CUSTOM PRO_MENU_21]/
		#endregion
