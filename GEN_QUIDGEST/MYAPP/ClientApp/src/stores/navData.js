...
import QNavContext from '@/mixins/navigationContext.js'
...
class NavigationState
{
	constructor()
	{
		let mainHistoryBranch = new Map(),
			mainHistory = new QNavContext.NavigationContext(MAIN_HISTORY_BRANCH_ID)
	...
	getHistory(historyBranchId)
	{
		...
		if (!this.history.has(historyBranchId))
		{
			...
			return new QNavContext.NavigationContext(historyBranchId)
		}
...
const actions = {
	...
	{
		this.navigation.history.set(navigationId, new QNavContext.NavigationContext(navigationId))
	},
