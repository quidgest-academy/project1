...
{
	public class Fotos_ProprValTitulo_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Fotos_ProprValTitulo"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Fotos_ProprValTitulo_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Fotos_ProprValTitulo_ViewModel(UserContext userContext) : base(userContext)
		{
			ValCodalbum = userContext.CurrentNavigation.CurrentLevel.GetEntry("album")?.ToString();
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Propr.AddEPH<CSGenioApropr>(ref u, crs, "IBL_FOTOS___PROPRTITULO__");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Propr.AddEPH<CSGenioApropr>(ref u, null, "IBL_FOTOS___PROPRTITULO__"));
			}
...
FieldRef[] fields = new FieldRef[] { CSGenioApropr.FldCodpropr, CSGenioApropr.FldZzstate, CSGenioApropr.FldTitulo };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_FOTOS___PROPRTITULO__");
			...
			conditions.SubSets.Add(fotos___proprtitulo__Conds);
			fotos___proprtitulo__Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PRO OVERRQ FOTOS_TITULO]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioApropr>(m_userContext, false, fotos___proprtitulo__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FOTOS___PROPRTITULO__", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PRO OVERRQLSTEXP FOTOS_TITULO]/
			...
			if (tableReload)
			{
// USE /[MANUAL PRO OVERRQLIST FOTOS_TITULO]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioApropr.GetInformation(), QMVC_POS_RECORD, sorts, fotos___proprtitulo__Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioApropr> listing = Models.ModelBase.Where<CSGenioApropr>(m_userContext, false, fotos___proprtitulo__Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_FOTOS___PROPRTITULO__", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapFotos_ProprValTitulo(listing);

				Menu.Identifier = "IBL_FOTOS___PROPRTITULO__";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_FOTOS___PROPRTITULO__";
		...
		private List<Models.Propr> MapFotos_ProprValTitulo(ListingMVC<CSGenioApropr> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapFotos_ProprValTitulo(row));
					i++;
				}
			}
		...
		private Models.Propr MapFotos_ProprValTitulo(CSGenioApropr row)
		{
		...
		#region Custom code
// USE /[MANUAL PRO VIEWMODEL_CUSTOM FOTOS_PROPRVALTITULO]/
		#endregion
