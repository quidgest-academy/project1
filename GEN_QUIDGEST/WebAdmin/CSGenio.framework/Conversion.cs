...
namespace CSGenio.framework
{
    ...
    public static class Conversion
    {
        ...
        public static decimal string2Numeric(string fieldValue, NumberFormatInfo provider)
        {
            ...
            {
                FrameworkException ef = new FrameworkException("Error converting from string to double.", "Conversion.string2Numeric", "Error converting from string to double: " + ex.Message, ex);
                return 0;
            }
        }
        ...
        public static int string2Int(object fieldValue)
        {
            ...
            {
                //new FrameworkException(null, "Conversion.string2Int", "Error converting from string to int: " + ex.Message, ex);
        ...
        public static DateTime dateString2DateTime(string dataString)
        {
            try
            {
                ...
                if (dataSplit.Length != 3)
                {
                    dataSplit = dataString.Split('-');
                    if (dataSplit.Length != 3)
                        throw new FrameworkException(null, "Conversion.dataString2DateTime", "String not recognized as a date: " + dataString);
                }
                ...
                if (Qresult.Success)
                {
                    ...
                    if (day.Equals("31") && (month.Equals("04") || month.Equals("06") || month.Equals("09") || month.Equals("11")))
                        throw new FrameworkException(null, "Conversion.dataString2DateTime", "The month " + month + " doesn't have 31 days.");
                    /*Fevereiro nao tem 30, nem 31*/
                    else if (day.CompareTo("30") >= 0 && month.Equals("02"))
                        throw new FrameworkException(null, "Conversion.dataString2DateTime", "February has less than 30 days.");
                    /*Se o Qyear é bissexto*/
                    else if (month.Equals("02") && day.Equals("29") && !(anoInt % 4 == 0 && (anoInt % 100 != 0 || anoInt % 400 == 0)))
                        throw new FrameworkException(null, "Conversion.dataString2DateTime", "February doesn't have 29 days.");
                }
                else
                    throw new FrameworkException(null, "Conversion.dataString2DateTime", "String not recognized as a date: " + dataString);
                DateTime data;
			...
			{
				if (ex.ExceptionSite != "Conversion.dataString2DateTime")
					//new FrameworkException(ex.UserMessage, "Converssao.dataString2DateTime", "Error converting from String to Datetime: " + ex.Message, ex);
					Log.Error(ex.UserMessage + " Converssao.dataString2DateTime Error converting from String to Datetime: " + ex.Message);
				return DateTime.MinValue;
			}
            catch (Exception ex)
            {
                //new FrameworkException(null, "Converssao.dataString2DateTime", "Error converting from String to Datetime: " + ex.Message, ex);
				Log.Error("Converssao.dataString2DateTime Error converting from String to Datetime: " + ex.Message);
                return DateTime.MinValue;
            }
        }
        ...
        public static DateTime dateTimeString2DateTime(string dataString)
        {
            try
            {
                ...
                {
                    throw new FrameworkException(null, "Conversion.datahoraString2DateTime", "String not recognized as a date: " + dataString);
                }
                ...
                if (Qresult.Success)
                {
                    ...
                    /*verificar se é um month de 30 days e o input de days é 31*/
                    if (day.Equals("31") && (month.Equals("04") || month.Equals("06") || month.Equals("09") || month.Equals("11")))
                        throw new FrameworkException(null, "Conversion.datahoraString2DateTime", "The month " + month + " doesn't have 31 days.");
                    /*Fevereiro nao tem 30, nem 31*/
                    else if (day.CompareTo("30") >= 0 && month.Equals("02"))
                        throw new FrameworkException(null, "Conversion.datahoraString2DateTime", "February has less than 30 days.");
                    /*Se o Qyear é bissexto*/
                    else if (month.Equals("02") && day.Equals("29") && !(anoInt % 4 == 0 && (anoInt % 100 != 0 || anoInt % 400 == 0)))
                        throw new FrameworkException(null, "Conversion.datahoraString2DateTime", "February desn't have 29 days.");

                    // MA 20101019 Se a hour é inválida
                    else if (hour > 23 || hour < 0 || minute > 59 || minute < 0 || second > 59 || second < 0)
                    {
                        throw new FrameworkException(null, "Conversion.datahoraString2DateTime", "Invalid time: " + dataString);
                    }
                }
                else
                    throw new FrameworkException(null, "Conversion.datahoraString2DateTime", "String not recognized as a date: " + dataString);
			...
			catch (Exception ex)
            {
                ...
                if(!(ex is FrameworkException))
                    Log.Error(string.Format("FrameWork Exception. [message] null [site] \"Conversion.dateTimeString2DateTime\" [cause] Error converting from String to Datetime: {0}", ex.Message));
                return DateTime.MinValue;
            }
        }
        ...
        public static string timeString2Time(string time)
        {
            ...
            if (tempoArr.Length != 2)
                throw new FrameworkException("Formato de tempo incorreto: " + time, "Conversion.tempoString2Tempo", "String not recognized as time: " + time);

            int horas, minutes;

            // parse das horas
            if (!int.TryParse(tempoArr[0], out horas))
                throw new FrameworkException("Formato de tempo incorreto: " + time, "Conversion.tempoString2Tempo", "Error parsing the hours: " + tempoArr[0]);

            // parse dos minutes
            if (!int.TryParse(tempoArr[1], out minutes))
                throw new FrameworkException("Formato de tempo incorreto: " + time, "Conversion.tempoString2Tempo", "Error parsing the minutes: " + tempoArr[1]);

            // validação dos Qvalues
            if (horas < 0 || horas > 23 || minutes < 0 || minutes > 59)
                throw new FrameworkException("Formato de tempo incorreto: " + time, "Conversion.tempoString2Tempo", "Hours and/or minutes outside accepted range: " + time);
        ...
        public static DateTime dateString2DateTime(string dataString, FieldFormatting formField)
        {
            try
            {
                ...
                if (dataSplit.Length != 3)
                {
                    dataSplit = dataString.Split('-');
                    if (dataSplit.Length != 3)
                        throw new FrameworkException(null, "Conversion.dataString2DateTime", "String not recognized as date: " + dataString);
                }
                DateTime data;
			...
			catch (Exception ex)
            {
                 ...
                 */
                if (!(ex is FrameworkException))
                    Log.Error(string.Format("FrameWork Exception. [message] null [site] \"Conversion.dateString2DateTime\" [cause] Error converting from string to DateTime: {0}", ex.Message));
        ...
        public static bool string2Bool(string fieldValue)
        {
            ...
            catch (Exception ex)
            {
                 ...
                 * We'll just log the error and return the default value.
                 */
                Log.Error(string.Format("FrameWork Exception. [message] {0} [site] \"Conversion.string2Bool\" [cause] Error converting from string to Boolean: {1}",
                    "Erro na conversão de string para booleano.", ex.Message));
                return false;
            }
        }
        ...
        public static object internal2InternalValid(object fieldValue, FieldFormatting forField)
        {
            ...
            throw new FrameworkException(null, "Conversion.interno2InternoValido", "Field format not known: " + forField.ToString());
        }
        ...
        public static int internalInt2InternalValidInt(object fieldValue)
        {
            ...
            else if (fieldValue is string)
            {
                ...
                throw new FrameworkException(null, "Conversion.internoInt2InternoValidoInt", "Error converting from database value to int: " + fieldValue);
        }
        ...
        public static decimal internalNumeric2InternalValidNumeric(object fieldValue)
        {
            ...
            else if (fieldValue is string)
            {
                ...
                throw new FrameworkException(null, "Conversion.internoNumeric2InternoValidNumeric", "Error converting from database value to double: " + fieldValue);
        }
        ...
        public static DateTime internalDateTime2InternalValidDateTime(object fieldValue)
        {
            ...
            if (fieldValue is DateTime)
                ...
                throw new FrameworkException(null, "Conversion.internoDateTime2InternoValidoDateTime", "Error converting from database value to DateTime: " + fieldValue);
        }
        ...
        public static string internalByteArray2DbBinary(object fieldValue)
        {
            ...
            else
            {
                if (fieldValue is Byte[])
                {
                    ...
                    throw new FrameworkException(null, "Conversion.tpInternoByteArray2tpBDBinario", "Error converting from internal type to string: " + fieldValue.ToString());
            }
        }
        ...
        public static string internal2String(object fieldValue, FieldType tpField)
        {
            try
            {
                ...
                switch (forField)
                {
                    ...
                    default:
                        throw new FrameworkException(null, "Conversion.tipoInterno2String", "The format is not defined: " + forField.ToString());
                }
            }
			catch (GenioException ex)
			{
				if (ex.ExceptionSite == "Conversion.tipoInterno2String")
					throw;
				throw new FrameworkException(ex.UserMessage, "Conversion.tipoInterno2String", "Error converting from internal type to string: " + ex.Message, ex);
			}
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Função que converte do tipo interno to strings recebidas em graficos flash
        /// </summary>
        /// <param name="valorCampo">Qvalue do Qfield no format interno</param>
        /// <param name="forCampo">formatação do Qfield</param>
        /// <returns>Qvalue do Qfield em string</returns>
        public static string internal2StringFlash(object fieldValue, FieldType tpField)
        {
            try
            {
                FieldFormatting forField = tpField.Formatting;
                switch (forField)
                {
                    ...
                    case FieldFormatting.DATASEGUNDO:
                        if (fieldValue is DateTime)
                            return dateTime2DateStringFlash((DateTime)fieldValue, FieldFormatting.DATAHORA);
                        else
                            return fieldValue.ToString();
                    case FieldFormatting.DATA:
                    case FieldFormatting.ANO_MES_DIA:
                    case FieldFormatting.DIA_MES_ANO:
                        return internalDateTime2StringFlash(fieldValue);
                    case FieldFormatting.TEMPO:
                    case FieldFormatting.CARACTERES:
                    case FieldFormatting.GUID:
                        return internalString2String(fieldValue);
                    case FieldFormatting.JPEG:
                        return fieldValue.ToString();
                    default:
                        throw new FrameworkException(null, "Conversion.tipoInterno2StringFlash", "The format is not defined: " + forField.ToString());
                }
            }
			catch (GenioException ex)
			{
				if (ex.ExceptionSite == "Conversion.tipoInterno2StringFlash")
					throw;
				throw new FrameworkException(ex.UserMessage, "Conversion.tipoInterno2StringFlash", "Error converting from internal type to string: " + ex.Message, ex);
			}
            catch (Exception ex)
            {
                throw new FrameworkException(null, "Conversion.tipoInterno2StringFlash", "Error converting from internal type to string: " + ex.Message, ex);
            }
        }
        ...
        public static string internal2String(object fieldValue, Type fieldType)
        {
            ...
            throw new FrameworkException(null, "Conversion.tipoInterno2String", "The type is not defined: " + fieldType.ToString());
        }
        ...
        /// <summary>
        /// Função que transforma um Qfield de tipo interno string em string aceite num report
        /// </summary>
        /// <param name="valorCampo">Qvalue Qfield</param>
        /// <returns>a string com o Qfield convertido to ser aceite na BD</returns>
        public static string internalString2StringReport(object fieldValue)
        {
            if (fieldValue == null || fieldValue == DBNull.Value)
                return "\"\"";
            else
        ...
        /// <summary>
        /// Função que transforma um Qfield de tipo interno string em string aceite num report to fields GUID
        /// </summary>
        /// <param name="valorCampo">Qvalue Qfield</param>
        /// <returns>a string com o Qfield convertido to ser aceite na BD</returns>
        public static string internalString2GUIDStringReport(object fieldValue)
        {
            if (fieldValue == null || fieldValue == DBNull.Value)
        ...
        public static string internalDateTime2String(object fieldValue)
        {
            ...
            if (fieldValue is string)
                ...
                throw new FrameworkException(null, "Conversion.tpInternoDateTime2String", "Error converting internal date type to string: " + fieldValue.ToString());
        }

        /// <summary>
        /// Função que transforma um Qfield de tipo interno DateTime em string aceite num flash
        /// </summary>
        /// <param name="valorCampo">Qvalue Qfield</param>
        /// <param name="formCampo">formatação do Qfield</param>
        /// <returns>a string com o Qfield convertido to ser aceite no flash</returns>
        public static string internalDateTime2StringFlash(object fieldValue)
        {
            if (fieldValue == null || fieldValue == DBNull.Value)
                return "";
            ...
            if (fieldValue is DateTime)
            {
                ...
                if (data.Equals(DateTime.MinValue))
                    ...
                    return Conversion.dateTime2DateStringFlash(data, FieldFormatting.ANO_MES_DIA);
            }
            if (fieldValue is string)
                return fieldValue.ToString();
            else
                throw new FrameworkException(null, "Conversion.tpInternoDateTime2StringFlash", "Error converting internal date type to string: " + fieldValue.ToString());
        }
        ...
        public static string dateTime2DateString(DateTime data, FieldFormatting formField)
        {
            ...
            {
                throw new FrameworkException(null, "Conversion.dateTime2DataString", "Error converting from DateTime to string: " + ex.Message, ex);
            }
        }

        /// <summary>
        /// Método que converte uma data no format string to format datetime dos graficos flash
        /// </summary>
        /// <param name="dataString"></param>
        /// <returns></returns>
        public static string dateTime2DateStringFlash(DateTime data, FieldFormatting formField)
        {
            try
            {
                if (data.Equals(DateTime.MinValue))
            ...
            {
                throw new FrameworkException(null, "Conversion.dateTime2DataStringFlash", "Error converting from DateTime to string: " + ex.Message, ex);
            }
        }

        /// <summary>
        /// Função que converte do tipo interno to string utilizada em reports
        /// </summary>
        /// <param name="valorCampo">Qvalue do Qfield no format interno</param>
        /// <param name="forCampo">formatação do Qfield</param>
        /// <returns>Qvalue do Qfield em string</returns>
        public static string internal2StringReport(object fieldValue, FieldType tpField)
        {
            try
            {
                FieldFormatting forField = tpField.Formatting;
                switch (forField)
                {
                    ...
                    case FieldFormatting.TEMPO:
                    case FieldFormatting.CARACTERES:
                        return internalString2StringReport(fieldValue);
                    case FieldFormatting.GUID:
                        return internalString2GUIDStringReport(fieldValue);
                    case FieldFormatting.JPEG:
                        return fieldValue.ToString();
                    default:
                        throw new FrameworkException(null, "Conversion.tipoInterno2StringReport", "Field format not known: " + forField.ToString());
                }
            }
			catch (GenioException ex)
			{
				if (ex.ExceptionSite == "Conversion.tipoInterno2StringReport")
					throw;
				throw new FrameworkException(ex.UserMessage, "Conversion.tipoInterno2StringReport", "Error converting from internal type to string: " + ex.Message, ex);
			}
            catch (Exception ex)
            {
                throw new FrameworkException(null, "Conversion.tipoInterno2StringReport", "Error converting from internal type to string: " + ex.Message, ex);
            }
        }
