...
describe('Slider', () => {
	it('renders correct input based on props', async () => {
		...
		var display = await wrapper.findAllByRole('value-display')
	...
	it('updates sliderValue when slider is moved', async () => {
		...
		var slider = await wrapper.findByRole('slider-input')
		...
		await fireEvent.input(slider, { target: { value: 97 } })
	...
	it('shows limits if showLimits prop is set to true', async () => {
		...
		var limits = await wrapper.queryByRole('display-limits')
