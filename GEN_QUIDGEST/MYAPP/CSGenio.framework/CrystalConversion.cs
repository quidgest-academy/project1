                ...
                switch (formatting)
                {
                    ...
                    case FieldFormatting.LOGICO:
                        ...
                        if (!int.TryParse(Qvalue.ToString(), out tmpInt))
							throw new FrameworkException(null, "CrystalConversion.FromInterno", "Error converting " + Qvalue.ToString() + " to int.");
                        return FromLogic(tmpInt);
                    case FieldFormatting.FLOAT:
                        if (!decimal.TryParse(Qvalue.ToString(), out decimal tmpDbl))
							throw new FrameworkException(null, "CrystalConversion.FromInterno", "Error converting " + Qvalue.ToString() + " to numeric.");
                        return FromNumeric(tmpDbl);
                    case FieldFormatting.DATA:
                        DateTime tmpDt1;
                        if (!DateTime.TryParse(Qvalue.ToString(), out tmpDt1))
                            throw new FrameworkException(null, "CrystalConversion.FromInterno", "Error converting " + Qvalue.ToString() + " to DateTime.");
                    ...
                    default:
						throw new FrameworkException(null, "CrystalConversion.FromInterno", "Format not recognized: " + formatting.ToString());
                }
            }
			catch (GenioException ex)
			{
				if (ex.ExceptionSite == "CrystalConversion.FromInterno")
					throw;
				throw new FrameworkException(ex.UserMessage, "CrystalConversion.FromInterno", "Error converting " + Qvalue.ToString() + ": " + ex.Message, ex);
			}
            catch (Exception ex)
            {
