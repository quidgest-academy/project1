...
{
	public class Cidade_PaisValPais_ViewModel : ListViewModel
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override string Uuid { get => "Cidade_PaisValPais"; }
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="Cidade_PaisValPais_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Cidade_PaisValPais_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public override List<Exports.QColumn> GetColumnsToExport(bool ajaxRequest = false)
		{
			...
			{
				new Exports.QColumn(CSGenioApais.FldPais, FieldType.TEXTO, Resources.Resources.PAIS58483, 50, 0, true),
			};
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.framework.TableConfiguration.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			{
				// EPH
				crs = Models.Pais.AddEPH<CSGenioApais>(ref u, crs, "IBL_CIDADE__PAIS_PAIS____");
			...
			crs.Criterias.Add(new Criteria(new ColumnReference(CSGenioApais.FldZzstate), CriteriaOperator.Equal, 0));


			if (tableReload)
			{
				string QMVC_POS_RECORD = requestValues["Q_POS_RECORD_pais"];
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Pais.AddEPH<CSGenioApais>(ref u, null, "IBL_CIDADE__PAIS_PAIS____"));
			}
...
FieldRef[] fields = new FieldRef[] { CSGenioApais.FldCodpais, CSGenioApais.FldZzstate, CSGenioApais.FldPais };
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_CIDADE__PAIS_PAIS____");
			...
			conditions.SubSets.Add(cidade__pais_pais____Conds);
			cidade__pais_pais____Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL PRO OVERRQ CIDADE_PAIS]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.Where<CSGenioApais>(m_userContext, false, cidade__pais_pais____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_CIDADE__PAIS_PAIS____", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL PRO OVERRQLSTEXP CIDADE_PAIS]/
			...
			if (tableReload)
			{
// USE /[MANUAL PRO OVERRQLIST CIDADE_PAIS]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioApais.GetInformation(), QMVC_POS_RECORD, sorts, cidade__pais_pais____Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioApais> listing = Models.ModelBase.Where<CSGenioApais>(m_userContext, false, cidade__pais_pais____Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_CIDADE__PAIS_PAIS____", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn);
				...
				Menu.Elements = MapCidade_PaisValPais(listing);

				Menu.Identifier = "IBL_CIDADE__PAIS_PAIS____";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_CIDADE__PAIS_PAIS____";
		...
		private List<Models.Pais> MapCidade_PaisValPais(ListingMVC<CSGenioApais> Qlisting)
		{
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapCidade_PaisValPais(row));
					i++;
				}
			}
		...
		private Models.Pais MapCidade_PaisValPais(CSGenioApais row)
		{
		...
		#region Custom code
// USE /[MANUAL PRO VIEWMODEL_CUSTOM CIDADE_PAISVALPAIS]/
		#endregion
