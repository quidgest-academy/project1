...
namespace CSGenio.business
{
    ...
    public class ArraysCrystalReports
    {
        ...
        static ArraysCrystalReports()
		{
			...
			// tipocons
            Qresult = new StringBuilder();
			            Qresult.AppendLine("if {{{0}}} = \"a\" then \"Apartamento\" else");
			            Qresult.AppendLine("if {{{0}}} = \"m\" then \"Moradia\" else");
			            Qresult.AppendLine("if {{{0}}} = \"o\" then \"Outra\" else");
            Qresult.Append("\"                                              \"");
            todasArrays.Add("tipocons", Qresult.ToString());
			// tipologi
            Qresult = new StringBuilder();
			Qresult.AppendLine("if {{{0}}} = 1 then \"T0\" else");
			Qresult.AppendLine("if {{{0}}} = 2 then \"T1\" else");
			Qresult.AppendLine("if {{{0}}} = 3 then \"T2\" else");
			Qresult.AppendLine("if {{{0}}} = 4 then \"T3 ou maior\" else");
            Qresult.Append("\"                                              \"");
            todasArrays.Add("tipologi", Qresult.ToString());
        }
